...
## 12. RAW_XML Benchmark Detaylı Sonuç Tabloları

Bu bölümde RAW_XML modunda yapılan benchmark denemeleri ayrı tablolar halinde verilmiştir.

Tablolarda kullanılan metrikler:
...
> Not: `samplesMs` ham ölçüm listesidir. Rapor okunabilirliğini korumak için bu bölümde ham `samplesMs` dizileri tabloya eklenmemiştir. Bu değerler API response çıktılarında mevcuttur ve ileride CSV/JSON export mekanizmasıyla ayrı dosyada tutulmalıdır.
...
```text
Bu test smoke test niteliğindedir.
Üç engine de hatasız çalışmıştır.
OpenSearch ve Solr tarafında bazı outlier değerleri görülmüştür.
```
...
```text
XML boyutu 34 KB seviyesinden 515 KB seviyesine çıkarıldığında Elasticsearch ve OpenSearch latency değerlerinde artış görülmüştür.
OpenSearch tarafında fatura itiraz sorgusunda yüksek outlier oluşmuştur.
Solr düşük latency üretmeye devam etmiştir.
```
...
```text
100 adet 515 KB XML dokümanı üzerinde üç engine de hatasız çalışmıştır.
OpenSearch bu testte önceki küçük teste göre daha stabil davranmıştır.
Solr hâlâ en düşük latency değerlerini üretmiştir.
```
...
```text
Tekil XML boyutu yaklaşık 1 MB seviyesine çıkarıldığında Elasticsearch ve OpenSearch latency değerleri artmıştır.
Solr tarafında bazı 0 ms ölçümleri görülmüştür; bu durum milisaniye çözünürlüğü nedeniyle 1 ms altındaki sürelerin 0 olarak görünmesinden kaynaklanabilir.
```
...
```text
Yaklaşık 2 MB XML boyutu ilk kez bu testte doğrulanmıştır.
Üç engine de 10 adet 2 MB XML dokümanı üzerinde hatasız çalışmıştır.
OpenSearch bu seviyede Elasticsearch’e göre daha yüksek latency üretmiştir.
```
...
```text
100 adet yaklaşık 2 MB XML dokümanı üzerinde Elasticsearch ve Solr hatasız çalışmıştır.
OpenSearch yalnızca ilk sorguda başarılı olmuş, sonraki iki sorguda tüm measurement iteration’larda hata üretmiştir.
Bu test, OpenSearch’ün 2 GB heap ile büyük RAW_XML search yükünde kararlı davranmadığını göstermiştir.
```
...
```text
50 adet 2 MB XML dokümanı seviyesinde de OpenSearch kararlı sonuç üretmemiştir.
Solr tarafında ilk sorguda yüksek outlier görülmüştür.
Bu nedenle OpenSearch’ün sistem genelinde kaynak baskısı oluşturup oluşturmadığını anlamak için izolasyon testi yapılmıştır.
```

---

### Test 8 — 50 Doküman x 2068 KB — Elasticsearch + Solr İzolasyon Testi
...
```text
OpenSearch devre dışı bırakıldığında Solr tarafındaki yüksek outlier değerleri belirgin şekilde azalmıştır.
Bu sonuç, önceki testte OpenSearch kaynaklı bellek/GC baskısının sistem genelindeki ölçümleri etkilemiş olabileceğini düşündürmektedir.
```

---

### Test 9 — 100 Doküman x 2068 KB — Elasticsearch + Solr İzolasyon Testi
...
```text
OpenSearch devre dışı bırakıldığında Elasticsearch ve Solr, 100 adet yaklaşık 2 MB XML dokümanı üzerinde RAW_XML benchmarkını hatasız tamamlamıştır.
Solr, bu izolasyon testinde Elasticsearch’e göre belirgin biçimde daha düşük latency üretmiştir.
Temel relevance kontrolünde iki engine’in aynı top-5 workflow sonuçlarını döndürmesi, Solr’ın düşük latency değerlerinin temel sonuç kalitesinden kopuk olmadığını göstermektedir.
```
